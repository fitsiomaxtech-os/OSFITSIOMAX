...
async def v3_move_consultation_stage(lead_id: str, payload: V3ConsultationStageInput, user: V3UserOut = Depends(v3_require_roles("branch_admin", "super_admin", "head_physio"))):
    ...
    if payload.consultation_stage == "Consultation Visit" and user.role != "super_admin":
        if user.role != "head_physio":
            raise HTTPException(
                status_code=403,
                detail="Only the consulting doctor can move a patient to Consultation Visit.",
            )
