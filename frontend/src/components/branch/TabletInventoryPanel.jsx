...
export const TabletInventoryPanel = () => {
  ...
  return (
    <div className="space-y-4" data-testid="tablet-inventory-panel">
      ...
      <Card data-testid="tablet-inventory-table-card">
        <CardContent className="p-4">
          <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Tablet Stock</p>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[720px] border-separate border-spacing-x-0 border-spacing-y-2 text-sm">
              <thead>
                <tr>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-slate-400">Tablet</th>
                  <th className="px-3 py-2 text-center text-[10px] font-semibold uppercase tracking-wider text-slate-400">Unit</th>
                  <th className="px-3 py-2 text-right text-[10px] font-semibold uppercase tracking-wider text-slate-400">Price</th>
                  <th className="px-3 py-2 text-center text-[10px] font-semibold uppercase tracking-wider text-slate-400">Stock</th>
                  <th className="px-3 py-2 text-right text-[10px] font-semibold uppercase tracking-wider text-slate-400">Actions</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  <tr><td colSpan={5} className="px-3 py-8 text-center text-sm text-slate-400">Loading stock...</td></tr>
                ) : visible.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-3 py-8 text-center text-sm text-slate-400">
                      {items.length === 0 ? "No tablets yet — add one to start tracking stock." : "Nothing matches that search."}
                    </td>
                  </tr>
                ) : visible.map((it) => (
                  <tr key={it.id} data-testid={`tablet-row-${it.id}`}>
                    <td className="rounded-l-[5px] border-y border-l border-slate-200 bg-white px-3 py-2">
                      <p className="font-semibold text-slate-800">{it.name}</p>
                      {it.brand && <p className="text-[11px] text-slate-400">{it.brand}</p>}
                    </td>
                    <td className="border-y border-slate-200 bg-white px-3 py-2 text-center text-slate-600">{it.unit}</td>
                    <td className="border-y border-slate-200 bg-white px-3 py-2 text-right font-semibold text-slate-800">{fmt(it.sale_price)}</td>
                    <td className="border-y border-slate-200 bg-white px-3 py-2 text-center"><StockBadge qty={it.stock} low={it.low} /></td>
                    <td className="rounded-r-[5px] border-y border-r border-slate-200 bg-white px-3 py-2">
                      <div className="flex flex-wrap items-center justify-end gap-1.5">
                        <Button
                          ...
                          onClick={() => setAddDraft({ item: it, qty: "", cost_price: "", supplier: "", note: "" })}
                          data-testid={`tablet-add-stock-${it.id}`}
                        >
                        ...
                        <Button
                          ...
                          disabled={it.stock <= 0}
                          onClick={() => setSellDraft({ item: it, qty: 1, unit_price: it.sale_price ?? "", customer_name: "", payment_mode: "cash", note: "" })}
                          data-testid={`tablet-sell-${it.id}`}
                        >
                        ...
                        <Button
                          ...
                          disabled={it.stock <= 0}
                          onClick={() => setMoveDraft({ item: it, qty: 1, to_branch_id: "", note: "" })}
                          data-testid={`tablet-move-${it.id}`}
                        >
                        ...
                        <button
                          ...
                          title="Edit" data-testid={`tablet-edit-${it.id}`}
                        >
                        ...
                        <button
                          ...
                          title="Delete" data-testid={`tablet-delete-${it.id}`}
                        >
      ...
      <Card data-testid="tablet-inventory-movements">
        <CardContent className="p-4">
              ...
              <table className="w-full min-w-[640px] text-sm">
                <thead>
                  <tr className="border-b border-slate-100">
                    <th className="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-slate-400">When</th>
                    <th className="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-slate-400">Tablet</th>
                    <th className="px-3 py-2 text-center text-[10px] font-semibold uppercase tracking-wider text-slate-400">What</th>
                    <th className="px-3 py-2 text-center text-[10px] font-semibold uppercase tracking-wider text-slate-400">Qty</th>
                    <th className="px-3 py-2 text-right text-[10px] font-semibold uppercase tracking-wider text-slate-400">Amount</th>
                    <th className="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-slate-400">Detail</th>
                  </tr>
                </thead>
                <tbody>
                  {movements.map((m) => {
                    ...
                    return (
                      <tr key={m.id} className="border-b border-slate-50 last:border-0" data-testid={`tablet-movement-${m.id}`}>
                        <td className="px-3 py-2 text-slate-500">{when(m.created_at)}</td>
                        <td className="px-3 py-2 font-medium text-slate-800">{m.item_name}</td>
                        <td className="px-3 py-2 text-center">
                          <span className={`inline-flex items-center rounded-[5px] border px-2 py-0.5 text-[10px] font-semibold ${meta.classes}`}>{meta.label}</span>
                        </td>
                        <td className="px-3 py-2 text-center font-semibold text-slate-700">{meta.sign}{m.qty}</td>
                        <td className="px-3 py-2 text-right text-slate-600">{m.kind === "sale" ? fmt(m.amount) : "—"}</td>
                        <td className="px-3 py-2 text-slate-500">
                          {detail}
                          {m.by_user_name && <span className="text-slate-300"> · {m.by_user_name}</span>}
                        </td>
                      </tr>
                    );
                  })}
